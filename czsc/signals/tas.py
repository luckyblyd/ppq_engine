...
def tas_macd_cross_V230705(c: CZSC, **kwargs) -> OrderedDict:
    """MACD指标窗口期内金死叉判断  贡献者：谌意勇

    参数模板："{freq}_D{di}F{fastperiod}L{slowperiod}S{signalperiod}n{n}_BS辅助V230704"

    **信号逻辑：**

    1. 如果前n期内MACD出现金叉且没有死叉，则发出买入信号（做多）。
    2. 如果前n期内MACD出现死叉且没有金叉，则发出卖出信号（做空）。

    **信号列表：**

    - Signal('15分钟_D1MACD12#26#9#n3_BS辅助V230705_金叉_任意_任意_0')
    - Signal('15分钟_D1MACD12#26#9#n3_BS辅助V230705_死叉_任意_任意_0')
    ...
    cache_key = update_macd_cache(c, **kwargs)
    freq = c.freq.value
    k1, k2, k3 = f"{freq}_D{di}F{fastperiod}L{slowperiod}S{signalperiod}n{n}_BS辅助V230704".split('_')
    v1 = "其他"

    if len(c.bars_raw) < n + di:
        return create_single_signal(k1=k1, k2=k2, k3=k3, v1=v1)

    macd = [x.cache[cache_key]['macd'] for x in c.bars_raw[-n - di:]]

    def cross_zero_single_axis(n: Union[List, np.ndarray]) -> int:
        """判断单个数列的零轴交叉点

        :param n: 数列
        :return:
        """
        axis_0 = np.zeros(len(n))
        n = np.flip(n)
        x = np.where(n[0] * n < axis_0, True, False)
        num = np.argmax(x[:-1] != x[1:]) + 2 if np.any(x) else 0
        return num

    num = cross_zero_single_axis(macd)
    if num >= n:
        if macd[-di] > 0 and macd[-num + 1] > 0:
            v1 = "金叉"
        elif macd[-di] < 0 and macd[-num + 1] < 0:
            v1 = "死叉"
...
def tas_macd_base_V230320(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    freq = c.freq.value
    k1, k2, k3 = f"{freq}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}MO{max_overlap}#{key}_BS辅助V230320".split(
        "_"
    )
    v1 = "其他"
...
def tas_cross_status_V230624(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    md = int(
        kwargs.get("md", 1)
    )  # md 是 min distance 的缩写，表示金死叉之间格距离小于此数值，将被忽略（去除一些杂波扰动因素,最小值不小于1）
    ...
    **信号列表：**
    ...
    if all(x < y for x, y in zip(c1_macd[-3:], c1_macd[-2:])) and min(c1_macd) < 0:
        ...
        if len(c2_bars) > 3:
            ...
            if (
                len(c2_gold_bars) == 1
                and len(c2_dead_bars) == 1
                and c2_gold_bars[-1].id - c2_dead_bars[-1].id >= 5
                and last_bar.dt == c2_gold_bars[0].dt > c2_dead_bars[0].dt
                and abs(min_macd) > abs(max_macd) * 0.3
            ):
    ...
    if all(x > y for x, y in zip(c1_macd[-3:], c1_macd[-2:])) and max(c1_macd) > 0:
        ...
        if len(c2_bars) > 3:
            ...
            if (
                len(c2_dead_bars) == 1
                and len(c2_gold_bars) == 1
                and c2_dead_bars[-1].id - c2_gold_bars[-1].id >= 5
                and last_bar.dt == c2_dead_bars[0].dt > c2_gold_bars[0].dt
                and abs(max_macd) > abs(min_macd) * 0.3
            ):
