...
def freq_end_date(dt, freq: Union[Freq, AnyStr]):
    ...
    if freq == Freq.Y:
        edt = dt.replace(month=12, day=31)
        return edt
...
class BarGenerator:
    ...
    def _update_freq(self, bar: RawBar, freq: Freq) -> None:
        """更新指定周期K线 - 优化版：减少 RawBar 创建次数"""
        fv = freq.value
        bars_list = self.bars[fv]
        freq_edt = freq_end_time(bar.dt, freq, self.market)

        if not bars_list:
            bars_list.append(RawBar(
                symbol=bar.symbol, freq=freq, dt=freq_edt, id=0,
                open=bar.open, close=bar.close, high=bar.high,
                low=bar.low, vol=bar.vol, amount=bar.amount))
            return

        last = bars_list[-1]
        if freq_edt != last.dt:
            bars_list.append(RawBar(
                symbol=bar.symbol, freq=freq, dt=freq_edt, id=last.id + 1,
                open=bar.open, close=bar.close, high=bar.high,
                low=bar.low, vol=bar.vol, amount=bar.amount))
        else:
            # 合并到已有K线 - 直接修改属性而非创建新对象
            ...
            if bar.low < last.low:
                last.low = bar.low
            last.vol += bar.vol
            last.amount += bar.amount

    def update(self, bar: RawBar) -> None:
        """更新各周期K线 - 优化版：预缓存freq对象，减少dict查找"""
        ...
        base_bars = self.bars[base_freq]
        if base_bars and base_bars[-1].dt == bar.dt:
            logger.warning(f"BarGenerator.update: 输入重复K线 {bar.dt}")
            return

        freq_map = self.freq_map
        for freq_str in self.bars.keys():
            self._update_freq(bar, freq_map[freq_str])

        # 限制内存中K线数量
        max_count = self.max_count
        for f, b in self.bars.items():
            if len(b) > max_count:
                self.bars[f] = b[-max_count:]
